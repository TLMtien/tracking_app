...
def VOLUME_PERFORMANCE(campain_id, all_outlet, from_date, to_date):
    ...
    average_volume = 0
    if count  > 0:
        average_volume = round(int(total_sale)/int(count), 0)
...
def get_outlet_province(campain_id, province, from_date, to_date):
    ...
    return all_outlet_province, list_type
...
def get_outlet_type_province(campain_id, list_province_type1, from_date, to_date):
    ...
    list_outlet = []
    list_outlet_by_type = []
    get_outlet_by_province = get_outlet_province(campain_id, list_province_type1, from_date, to_date)[0]
    if len(get_outlet_by_province) > 0:
        for outlet in get_outlet_by_province:
            if outlet.type in list_province_type1 and not outlet in list_outlet_by_type :
                list_outlet_by_type.append(outlet)
        if len(list_outlet_by_type) > 0:  #type exist 
            list_outlet = list_outlet_by_type
        else:      #type not exist
            list_outlet = get_outlet_by_province 
    else:
        for type in list_province_type1:  
            Cp = Campain.objects.get(id=campain_id)
            sale_person = SalePerson.objects.filter(brand__pk=campain_id)  # all_SP
            for SP in sale_person:
                ...
                if rp_table.exists() or rp_sale.exists() or gift_rp.exists():
                    ...
                    for rp in report:
                        outlet = rp.outlet
                        if not outlet in list_outlet and outlet.created_by_HVN and outlet.type == type:
                            list_outlet.append(outlet)
    return list_outlet, list_province
# def get_outlet_type_province(campain_id, list_province_type1, from_date, to_date):
#     Cp = Campain.objects.get(id = campain_id)
...
#                         all_outlet_province.append(outlet) #get all outlet by province
#         # end filter outlet
...
def get_outletName_type_province(campain_id, list_province_type_outletname, from_date, to_date):
    ...
    list_province = []
    list_outlet = []
    list_outlet_by_outletName = []
    get_outlet_by_type = get_outlet_type_province(campain_id, list_province_type_outletname, from_date, to_date)[0]
    if len(get_outlet_by_type) > 0:
        for outlet in get_outlet_by_type:
            if outlet.outlet_Name in list_province_type_outletname and not outlet in list_outlet_by_outletName :
                list_outlet_by_outletName.append(outlet)
        if len(list_outlet_by_outletName) > 0:  #type exist 
            list_outlet = list_outlet_by_outletName
        else:      #type not exist
            list_outlet = get_outlet_by_type
    else:
        for outlet_name in list_province_type_outletname:  
            Cp = Campain.objects.get(id=campain_id)
            sale_person = SalePerson.objects.filter(brand__pk=campain_id)  # all_SP
            for SP in sale_person:
                ...
                if rp_table.exists() or rp_sale.exists() or gift_rp.exists():
                    ...
                    for rp in report:
                        outlet = rp.outlet
                        if not outlet in list_outlet and outlet.created_by_HVN and outlet.outlet_Name == outlet_name:
                            list_outlet.append(outlet)
    return list_outlet, list_province
# def get_outletName_type_province(campain_id, list_province_type_outletname, from_date, to_date):
#      Cp = Campain.objects.get(id = campain_id)
